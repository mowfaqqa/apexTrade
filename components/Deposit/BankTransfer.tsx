...
const BankTransfer = () => {
    const depositData = collection(db, "dashboard", "deposits", "bitcoin")
    const createBankTransferDepositRequest = async (values: any) => {
        await setDoc(doc(depositData) ,values)
    }
    const formik = useFormik({
        ...
        onSubmit : (values) => {
            createBankTransferDepositRequest(values).then(() => notifySuccess("Request Sent Sucessfully")).catch(() => notifyError("Deposit Request Failed !!"));
        }
    })
  return (
    <div className='max-w-7xl mx-auto'>
        ...
        <p className='px-2 mb-2'>To make payment from your trading account please use our wallet address above and click on <span className='text-orange-500'>Make payment</span></p>
        <div>
